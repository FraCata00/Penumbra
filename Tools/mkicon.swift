...
// Night: the lower-right half, split off by a diagonal
ctx.saveGState()
...
ctx.clip()                                   // stay inside the moon disc
ctx.addEllipse(in: moon)
ctx.addEllipse(in: moon.offsetBy(dx: 52, dy: 36))
ctx.clip(using: .evenOdd)                    // ...minus the offset disc = crescent
...
print("wrote 1024px icon:", out.path)
